...
richuser-b3 3.0 _ ITZME RICHUSER/BLACKCAT
...
const {Message, StringSession, Image, Video} = require('./richuser-b3/');
...
// Sql
const richuser-b3DB = config.DATABASE.define('richuser-b3', {
    info: {
...
async function richuser-b3 () {
    await config.DATABASE.sync();
    var StrSes_Db = await richuser-b3DB.findAll({
        where: {
          info: 'StringSession'
        }
    });
    ...
    conn.on ('credentials-updated', async () => {
        ...
        if (StrSes_Db.length < 1) {
            await richuser-b3DB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
...
*/
                        try {
                            ...
                            if (config.LANG == 'EN') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/richuser.png"), MessageType.image, { caption: '*『 ERROR 』*\n\n*richuser an error has occurred!*\n_Report this error to the developer! [ TERROR BOY ]._\n\n*Error:* ```' + error + '```\n\n' });
                                
                            } else if (config.LANG == 'ML') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/richuser.png"), MessageType.image, { caption: '*『 ERROR 』*\n\n*richuser error സംഭവിച്ചു!*\n_ഈ error  ഡെവലപ്പറെ അറിയിക്കുക! [ TERROR BOY ]._\n\n*error:* ```' + error + '```\n\n' });
                                
                            } else {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/richuser.png"), MessageType.image, { caption: '*『 KESALAHAN 』*\n\n*richuser telah terjadi kesalahan!*\n_Laporkan kesalahan ini ke pengembang [ TERROR BOY ]._\n\n*Kesalahan:* ```' + error + '```\n\n' });
                            }
                        }
                    }
                }
